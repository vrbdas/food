...
function calc() {
  var result = document.querySelector('.calculating__result span'); // ваша суточная норма калорий:
  ...
  function initLocalSettings(selector, activeClass) {
    // устанавливает значения и классы активности в соответствии с localStorage
    var elements = document.querySelectorAll(selector);
    if (localStorage.getItem('sex')) {
      // если в localStorage есть значение, устанавливает его
    ...
    document.querySelectorAll('.calculating__choose-item').forEach(function (item) {
    ...
    elements.forEach(function (elem) {
      ...
      if (elem.getAttribute('id') === localStorage.getItem('sex')) {
        // устанавливает классы активности на блоки со значениями как в localStorage
  ...
  function calcTotal() {
    // вычисляет калории и показывает результат
  ...
  function getStaticInformation(selector, activeClass) {
    // получает значения пол и физическая активность
    var elements = document.querySelectorAll(selector); // получает все блоки внутри parentSelector

    elements.forEach(function (item) {
      item.addEventListener('click', function (event) {
        if (event.target.getAttribute('data-ratio')) {
          // если у объекта события есть аттрибут data-ratio="..."
        ...
        elements.forEach(function (elem) {
          // убирает класс активности у всех блоков
  ...
  function getDynamicInformation(selector) {
    // получает значения ваша конституция
...
function cards() {
  ...
  var menu = [
  // вставил этот масссив вместо ответа от JSON-сервера
  {
  ...
  function createCard(data) {
    data.forEach(function (_ref) {
      var img = _ref.img,
        ...
        descr = _ref.descr,
        price = _ref.price;
      // Для каждого объекта из массива menu разбивает его на значения и они присваиваются переменным с таким же именем
...
function forms() {
  ...
  var modalBlock = document.querySelector('.modal'); // изначально в html стоит класс .hide

  formTags.forEach(function (item) {
    // На каждую форму вешает обработчик формы
    bindPostData(item);
  });
  function bindPostData(form) {
    form.addEventListener('submit', function (event) {
      // Событие отправка формы кликом на кнопку или enter
      event.preventDefault();
      var spinner = document.createElement('img'); // К форме добавляется спиннер загрузки
      ...
      var formData = new FormData(form); // Данные из формы, во всех input обязательно должны быть аттрибуты name=""
      ...
      (0,_services_services__WEBPACK_IMPORTED_MODULE_8__.postData)('http://localhost:3000/requests', json) // Настраивает и посылает запрос на сервер
      .then(function (data) {
        // Обработка успешного promise
  ...
  function showThanksModal(text) {
    // Меняет модальное окно на сообщение об отправке
    var formModalDialog = document.querySelector('.modal__dialog');
    formModalDialog.classList.add('hide'); // Скрывает внутреннюю часть старого окна
    (0,_modal__WEBPACK_IMPORTED_MODULE_7__.modalShow)('.modal'); // Показывает модальное окно с пустой внутренней частью

    var thanksModalDialog = document.createElement('div'); // Создает внутреннюю часть с текстом text
    ...
    setTimeout(function () {
      // Через 4с скрывает окно и возвращает внутреннюю часть с формой
      thanksModalDialog.remove();
      formModalDialog.classList.remove('hide');
      formModalDialog.classList.add('show');
      (0,_modal__WEBPACK_IMPORTED_MODULE_7__.modalHide)('.modal');
    }, 4000);
  }
}
...
function modalShow(modalBlockSelector, modalTimerId) {
  var modalBlock = document.querySelector(modalBlockSelector); // Изначально в html стоит класс hide
  ...
  if (modalTimerId) {
    // Если задан таймер на автоматическое открытие окна
...
function modal(modalBlockSelector, modalTriggerSelector, modalTimerId) {
  var modalBlock = document.querySelector(modalBlockSelector); // Изначально в html стоит класс hide
  var modalTrigger = document.querySelectorAll(modalTriggerSelector); // Кнопка для вызова модального окна

  modalTrigger.forEach(function (item) {
    // Показавает окно при клике на кнопки 'Связаться с нами'
    item.addEventListener('click', function () {
      return modalShow(modalBlockSelector, modalTimerId);
    });
  });
  ...
  modalBlock.addEventListener('click', function (event) {
    // Закрывает окно при клике на область вокруг .modal__dialog или на крестик
...
function slider(_ref) {
  var sliderBlockSelector = _ref.sliderBlockSelector,
    ...
    navigationSelector = _ref.navigationSelector,
    dotSelector = _ref.dotSelector,
    nextArrowSelector = _ref.nextArrowSelector,
    prevArrowSelector = _ref.prevArrowSelector;
  var sliderBlock = document.querySelector(sliderBlockSelector); // '.offer__slider'
  var current = document.querySelector(currentSelector); // '#current'
  var total = document.querySelector(totalSelector); // '#total'
  var inner = document.querySelector(innerSelector); // '.offer__slider-inner'
  var wrapper = document.querySelector(wrapperSelector); // '.offer__slider-wrapper'
  var slides = document.querySelectorAll(slideSelector); // клоны сюда не попадут, это статический nodelist // '.offer__slide'
  var nav = document.querySelector(navigationSelector); // блок с точками // '.offer__slider-navigation'

  var transition = 0.75; // время плавного перехода между слайдами в секундах
  var wrapperWidth = parseInt(window.getComputedStyle(wrapper).width, 10); // parseInt отбрасывает 'px'
  var threshold = wrapperWidth * 0.33; // порог, после которого переключается слайд = треть от ширины одного слайда
  var mouseStart = 0; // начальное положение мыши
  var mouseMove = 0; // смещение мыши
  var initialPos; // позиция слайдера перед началом перетаскивания
  var dots; // точки
  var slideIndex = 1; // начальные значения
  var canSlide = true; // начальные значения
  ...
  function getZero(num) {
    // Если число из одной цифры, добавляет перед ним 0
    if (num >= 0 && num < 10) {
      return "0".concat(num);
    }
    return num;
  ...
  function sliderInitialization() {
    // создание элементов слайдера и задание начального положения
    var firstSlideClone = document.createElement('div'); // создает клон первого слайда и помещает его в конец
    firstSlideClone.classList.add("".concat(slideSelector.slice(1))); // slice убирает точку в начале
    firstSlideClone.innerHTML = slides[0].innerHTML;
    inner.append(firstSlideClone);
    var lastSlideClone = document.createElement('div'); // создает клон последнего слайда и помещает его в начало
    lastSlideClone.classList.add("".concat(slideSelector.slice(1))); // slice убирает точку в начале
    lastSlideClone.innerHTML = slides[slides.length - 1].innerHTML;
    inner.prepend(lastSlideClone);
    for (var i = 0; i < slides.length; i += 1) {
      // создает точки по количеству слайдов
      var dot = document.createElement('div');
      dot.classList.add("".concat(dotSelector.slice(1))); // slice убирает точку в начале
      nav.append(dot);
      dots = document.querySelectorAll(dotSelector); // '.offer__slider-dot'
    }

    inner.style.width = "".concat(wrapperWidth * (slides.length + 2), "px"); // устанавливает ширину ленты со слайдами, +2 это два клона, которых нет в slides
  ...
  function clickDelay() {
    // следующий клик можно сделать только через указанное время
    canSlide = false;
    setTimeout(function () {
      canSlide = true;
  ...
  function currentAndDots() {
    // показывает номер слайда и делает активной точку
    current.textContent = getZero(slideIndex); // getZero добавляет ноль, если число из 1 цифры
    dots.forEach(function (dot) {
      return dot.classList.remove("".concat(dotSelector.slice(1), "_active"));
    }); // удаляет класс активности со всех точек
    dots[slideIndex - 1].classList.add("".concat(dotSelector.slice(1), "_active")); // показывает точку по индексу
  }

  function showSlide() {
    // показывает слайд по индексу
    inner.style.transform = "translateX(".concat(-wrapperWidth * slideIndex, "px)"); // показывает слайд
  }

  function addAnimation() {
    inner.style.transition = "all ".concat(transition, "s"); // добавляет анимацию плавного перелистывания
  }

  function removeAnimation() {
    inner.addEventListener('transitionend', function () {
      return inner.style.transition = 'none';
    }); // ждет окончания анимации и убирает анимацию;
  }
  ...
  function nextSlide() {
    ...
    if (slideIndex > slides.length) {
      // долистал до последнего слайда
      slideIndex = 1; // счетчик на первый слайд
      inner.style.transform = "translateX(".concat(-wrapperWidth * (slides.length + 1), "px)"); // показывает клон 1го
      inner.addEventListener('transitionend', function () {
        // ждет окончания анимации
        inner.style.transition = 'none'; // убирает анимацию, чтобы незаметно перейти на первый слайд
        inner.style.transform = "translateX(".concat(-wrapperWidth * slideIndex, "px)"); // переходит на первый слайд
      });
    } else {
      showSlide(); // показывает слайд по индексу
  ...
  function prevSlide() {
    ...
    slideIndex -= 1;
    if (slideIndex < 1) {
      // долистал до 1 слайда
      slideIndex = slides.length; // счетчик на последний слайд
      inner.style.transform = "translateX(".concat(0, "px)"); // показывает клон последнего слайда
      inner.addEventListener('transitionend', function () {
        // ждет окончания анимации
        inner.style.transition = 'none'; // убирает анимацию, чтобы незаметно перейти на последний слайд
        inner.style.transform = "translateX(".concat(-wrapperWidth * slideIndex, "px)"); // переходит на последний слайд
      });
    } else {
      showSlide(); // показывает слайд по индексу
      removeAnimation(); // ждет окончания анимации и убирает анимацию;
  ...
  function sliderBtnsClicks() {
    // клик на кнопки и точки
    sliderBlock.addEventListener('click', function (event) {
      event.preventDefault();
      if (canSlide === true) {
        // не запускает обработчик, пока не прошла задержка
        if (event.target && event.target.closest(nextArrowSelector) && sliderBlock.contains(event.target.closest(nextArrowSelector))) {
          ...
          prevSlide();
        } else if (event.target && event.target.closest(dotSelector) && sliderBlock.contains(event.target.closest(dotSelector))) {
          // клик на точки
          addAnimation(); // добавляет анимацию плавного перелистывания
          dots.forEach(function (item, i) {
            if (item === event.target) {
              // находит точку, на которую нажали
              slideIndex = i + 1;
              currentAndDots(); // показывает номер слайда и делает активной точку
              showSlide(); // показывает слайд по индексу
              removeAnimation(); // ждет окончания анимации и убирает анимацию;
  ...
  function sliderDragActions() {
    // перетаскивание для слайдера
  ...
  function dragAction(event) {
    ...
    var currentTransform = +inner.style.transform.match(/[-0-9.]+/)[0];
    ...
    inner.style.transform = "translateX(".concat(currentTransform - mouseMove, "px)");
  }
  function dragEnd() {
    var finalPos = +inner.style.transform.match(/[-0-9.]+/)[0]; // translateX в конце перетаскивания

    if (-finalPos - -initialPos === 0) {
      // если не сдвигали слайдер, просто кликнули
      inner.style.transition = 'none';
    } else if (-finalPos - -initialPos > threshold) {
      // если смещение больше порога и положительное, переключает на следующий слайд
      nextSlide();
    } else if (-finalPos - -initialPos < -threshold) {
      // если смещение больше порога и отрицательное, переключает на предыдущий слайд
      prevSlide();
    } else {
      // если смещение меньше порога, возвращает к тому положению, где была зажата ЛКМ
      inner.style.transition = "all ".concat(transition, "s"); // добавляет анимацию плавного перелистывания
      inner.style.transform = "translateX(".concat(initialPos, "px)");
      inner.addEventListener('transitionend', function () {
        return inner.style.transition = 'none';
      }); // ждет окончания анимации и убирает анимацию;
...
function tabs(tabContentBlockSelector, tabHeaderContainerSelector, tabHeaderItemSelector) {
  var tabContentBlocks = document.querySelectorAll(tabContentBlockSelector); // Все вкладки
  var tabHeaderContainer = document.querySelector(tabHeaderContainerSelector); // Контейнер с заголовками вкладок
  var tabHeaderItems = document.querySelectorAll(tabHeaderItemSelector); // Все заголовки вкладок

  hideTabContent(); // Скрывает все вкладки
  showTabContent(); // Показывает первую вкладку по умолчанию (i = 0 в функции showTabContent)

  tabHeaderContainer.addEventListener('click', function (event) {
    // Обрабатывает клик на контейнер с вкладками
    if (event.target && event.target.matches(tabHeaderItemSelector)) {
      // Проверяет клик на заголовок вкладки
  ...
  function hideTabContent() {
    ...
    tabContentBlocks.forEach(function (item) {
      item.classList.add('hide'); // display: none;
      item.classList.remove('show', 'fade');
    });
    tabHeaderItems.forEach(function (item) {
      item.classList.remove("".concat(tabHeaderItemSelector.slice(1), "_active"));
  ...
  function showTabContent() {
    ...
    // Показывает выбранную вкладку
    tabContentBlocks[i].classList.add('show', 'fade'); // display: block;
    tabContentBlocks[i].classList.remove('hide');
    tabHeaderItems[i].classList.add("".concat(tabHeaderItemSelector.slice(1), "_active"));
...
function timer() {
  ...
  function promotionTime() {
    ...
    if (localStorage.getItem('deadline') && Date.parse(localStorage.getItem('deadline')) > Date.parse(new Date())) {
      ...
      // если в локальном хранилище нет даты окончания акции или она просрочена
      var deadlineCalc = new Date(Date.now() + 1 * 24 * 60 * 60 * 1000); // текущая дата в текущем часовом поясе + 1д
    ...
    var promotionEndText = new Date(deadline).toLocaleString('ru', {
      day: '2-digit',
      month: 'long'
    }); // выводит дату в виде 2 цифр и месяц в виде слова, склоняя на русском языке

    document.querySelector('#promotion-end').textContent = "\u0410\u043A\u0446\u0438\u044F \u0437\u0430\u043A\u043E\u043D\u0447\u0438\u0442\u0441\u044F ".concat(promotionEndText, " \u0432 00:00"); // добавляет текст на страницу
  }

  function getTimeRemaining(endtime) {
    // Вычисляет оставшееся время от текущей даты до endtime
    var t = Date.parse(endtime) - Date.parse(new Date()); // Вычитает текущую дату из конечной в мс
    var days = Math.floor(t / (1000 * 60 * 60 * 24)); // мс в дни
    var hours = Math.floor(t / (1000 * 60 * 60) % 24); // мс в часы как остаток от деления на 24
    var minutes = Math.floor(t / (1000 * 60) % 60); // мс в минуты как остаток от деления на 60
    var seconds = Math.floor(t / 1000 % 60); // мс в секунды как остаток от деления на 60

    return {
      // Возвращает оставшееся время как объект
      total: t,
      days: days,
      hours: hours,
      minutes: minutes,
  ...
  function setClock(timerBlock, endtime) {
    ...
    var timer = document.querySelector(timerBlock); // Контейнер с блоками таймера
    var days = timer.querySelector('#days'); // Блок с днями
    var hours = timer.querySelector('#hours'); // Блок с часами
    var minutes = timer.querySelector('#minutes'); // Блок с минутами
    var seconds = timer.querySelector('#seconds'); // Блок с секундами

    var timeInterval = setInterval(updateClock, 1000); // Обновление таймера будет запускаться каждую секунду

    updateClock(); // Запускает функцию сразу, чтобы не ждать 1с до первого обновления

    function updateClock() {
      // Обновление таймера
      var t = getTimeRemaining(endtime); // Создает объект с оставшимся временем
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          _context.next = 2;
          return fetch(url, {
            // await дождется результата функции fetch
            method: 'POST',
            // POST это отправка, GET получение
            headers: {
              'Content-type': 'application/json'
            },
            // Заголовки нужны для JSON, если на сервер отправлять formData, то не нужны
